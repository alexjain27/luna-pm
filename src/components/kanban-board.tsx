...
export function KanbanBoard({
  ...
  return (
    <div className="w-full overflow-x-auto overflow-y-hidden">
      <div
        className="grid gap-4 min-w-max items-start"
        style={{ gridTemplateColumns: `repeat(${statuses.length}, minmax(240px, 1fr))` }}
      >
        {statuses.map((status) => {
          const statusTasks = tasks.filter((t) => t.statusId === status.id);
          return (
            <div key={status.id} className="flex flex-col gap-2 max-h-[calc(100vh-220px)] overflow-y-auto pr-1">
              <div className="sticky top-0 bg-background flex items-center gap-2 pb-2 border-b-2 z-10" style={{ borderColor: status.color }}>
